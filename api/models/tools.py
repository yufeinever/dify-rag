...
class GeneratedFile(TypeBase):
    """Durable index for local ToolFiles and trusted remote generated assets.

    ``owner_user_id`` may reference either a console Account or an EndUser. Reads must
    therefore scope by tenant and the authenticated actor rather than relying on a
    foreign key to one identity table.
    """
    ...
    __table_args__ = (
        ...
        sa.UniqueConstraint("tenant_id", "source_key", name="generated_file_tenant_source_key_unique"),
        ...
        sa.Index("generated_file_tenant_source_kind_created_at_idx", "tenant_id", "source_kind", "created_at"),
        sa.Index("generated_file_source_app_idx", "source_app_id"),
        sa.Index("generated_file_source_workflow_run_idx", "source_workflow_run_id"),
    )
    ...
    tool_file_id: Mapped[str | None] = mapped_column(StringUUID, nullable=True, default=None)
    storage_type: Mapped[str] = mapped_column(String(32), nullable=False, default="tool_file")
    source_url: Mapped[str | None] = mapped_column(LongText, nullable=True, default=None)
    thumbnail_url: Mapped[str | None] = mapped_column(LongText, nullable=True, default=None)
    ...
    source_workflow_run_id: Mapped[str | None] = mapped_column(StringUUID, nullable=True, default=None)
    source_kind: Mapped[str] = mapped_column(String(64), nullable=False, default="message_file")
    ...
    asset_metadata: Mapped[dict[str, Any]] = mapped_column(
        sa.JSON, nullable=False, server_default=sa.text("'{}'"), default_factory=dict
    )
