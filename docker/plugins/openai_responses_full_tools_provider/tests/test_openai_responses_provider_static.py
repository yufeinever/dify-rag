...
class OpenAIResponsesProviderStaticTests(unittest.TestCase):
    ...
    def test_llm_converts_dify_documents_to_responses_input_files(self):
        source = (ROOT / "models" / "llm" / "llm.py").read_text()
        ast.parse(source)
        self.assertIn("DocumentPromptMessageContent", source)
        self.assertIn("PromptMessageContentType.DOCUMENT", source)
        self.assertIn('"type": "input_file"', source)
        self.assertIn('"file_data": f"data:{file.mime_type};base64,{file.base64_data}"', source)

    def test_gpt_5_5_declares_document_support(self):
